...
class PokemonParser:
    ...
    def process_pokemon_data(self, game_names, base_dir='games'):
        """Process Pokémon data for multiple games."""
        for game_name in game_names:
            print(f"Processing game: {game_name}")
            
            pokemon_txt_file = os.path.join(base_dir, game_name, 'data', 'pokemon.txt')
            pokemon_json_file = os.path.join(base_dir, game_name, 'data', 'pokemon.json')
            #print(pokemon_txt_file)
            
            if not os.path.exists(pokemon_txt_file):
                print(f"Pokémon file not found for game: {game_name}")
                continue
            
            self.convert_txt_to_json(pokemon_txt_file, pokemon_json_file)
            print(f"Pokémon data converted for game: {game_name}")

if __name__ == '__main__':
    parser = PokemonParser()
    parser.process_pokemon_data(['vanguard', 'ss2'])